"""
Drone detection on MacBook using trained YOLO (ONNX)
Uses CoreML (Apple GPU) via onnxruntime when available
...
MODEL_PATH = Path("best_new.onnx")
IMG_SIZE = 640
CONF_THRESHOLD = 0.5
IOU_THRESHOLD = 0.5
MAX_BOX_RATIO = 0.6  # reject boxes covering more than 60% of frame width or height
...
def postprocess(output: np.ndarray, scale: float, conf_thresh: float, iou_thresh: float,
    ...
    # YOLOv8/11 ONNX output: [1, 5, 8400] = [cx, cy, w, h, score] transposed
    ...
    for i in keep:
        ...
        # Filter out absurdly large boxes (false positives covering most of frame)
        if frame_w > 0 and frame_h > 0:
            if bw / frame_w > MAX_BOX_RATIO or bh / frame_h > MAX_BOX_RATIO:
                continue
        results.append((boxes[i].astype(int).tolist(), float(scores[i]), 0))
    return results
...
def run(source: str) -> None:
    ...
    if is_image:
        ...
        detections = postprocess(output, scale, CONF_THRESHOLD, IOU_THRESHOLD, fw, fh)
